...
void TMX_Image::Load_Image(char *data, xml_node<> *node)
{
    ...
    if(xml_attribute<> *iattr = node->first_attribute("width"))    Width    = strtoul( iattr->value(), NULL, 0 );
    if(xml_attribute<> *iattr = node->first_attribute("height"))   Height   = strtoul( iattr->value(), NULL, 0 );
